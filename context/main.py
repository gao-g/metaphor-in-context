...
'''
raw_train_trofi = []
with open('../data/TroFi/TroFi_formatted_train.csv') as f:
    lines = csv.reader(f)
    next(lines)
    for line in lines:
        raw_train_trofi.append([line[1], int(line[2]), int(line[3])])

raw_val_trofi = []
with open('../data/TroFi/TroFi_formatted_val.csv') as f:
    lines = csv.reader(f)
    next(lines)
    for line in lines:
        raw_val_trofi.append([line[1], int(line[2]), int(line[3])])

raw_test_trofi = []
with open('../data/TroFi/TroFi_formatted_test.csv') as f:
    lines = csv.reader(f)
    next(lines)
    for line in lines:
        raw_test_trofi.append([line[1], int(line[2]), int(line[3])])
print('TroFi dataset division: ', len(raw_train_trofi), len(raw_val_trofi), len(raw_test_trofi))
...
vocab = get_vocab(raw_train_vua + raw_val_vua + raw_test_vua +
                  raw_train_trofi + raw_val_trofi + + raw_test_trofi + raw_mohX)
...
elmos_train_trofi = h5py.File('../elmo/TroFi_train.hdf5', 'r')
elmos_val_trofi = h5py.File('../elmo/TroFi_val.hdf5', 'r')
...
embedded_train_trofi = [[embed_sequence(example[0], example[1], word2idx,
                                        glove_embeddings, elmos_train_trofi, suffix_embeddings), example[2]]
                        for example in raw_train_trofi]
embedded_val_trofi = [[embed_sequence(example[0], example[1], word2idx,
                                      glove_embeddings, elmos_val_trofi, suffix_embeddings), example[2]]
                      for example in raw_val_trofi]
...
train_dataset_trofi = TextDataset([example[0] for example in embedded_train_trofi],
                                  [example[1] for example in embedded_train_trofi])
val_dataset_trofi = TextDataset([example[0] for example in embedded_val_trofi],
                                [example[1] for example in embedded_val_trofi])

dataset_mohX = TextDataset([example[0] for example in embedded_mohX],
                           [example[1] for example in embedded_mohX])
...
train_dataloader_trofi = DataLoader(dataset=train_dataset_trofi, batch_size=batch_size, shuffle=True,
                                    collate_fn=TextDataset.collate_fn)
val_dataloader_trofi = DataLoader(dataset=val_dataset_trofi, batch_size=batch_size,
                                  collate_fn=TextDataset.collate_fn)

dataloader_mohX = DataLoader(dataset=dataset_mohX, batch_size=batch_size,
                             collate_fn=TextDataset.collate_fn)
"""
# gather all 3 datasets
embedded_train_all = embedded_train_trofi + embedded_train_vua
embedded_val_all = embedded_val_trofi + embedded_val_vua

train_dataset_all = TextDataset([example[0] for example in embedded_train_all],
                                [example[1] for example in embedded_train_all])
val_dataset_all = TextDataset([example[0] for example in embedded_val_all],
                              [example[1] for example in embedded_val_all])

train_dataloader_all = DataLoader(dataset=train_dataset_all, batch_size=batch_size, shuffle=True,
                              collate_fn=TextDataset.collate_fn)
val_dataloader_all = DataLoader(dataset=val_dataset_all, batch_size=batch_size,
                            collate_fn=TextDataset.collate_fn)
"""
...
"""
4. test the model
the following code is for test data of vua and trofi
"""
'''
VUA
'''
...
TroFi
'''
elmos_test_trofi = h5py.File('../elmo/TroFi_test.hdf5', 'r')
embedded_test_trofi = [[embed_sequence(example[0], example[1], word2idx,
                                       glove_embeddings, elmos_test_trofi, suffix_embeddings), example[2]]
                       for example in raw_test_trofi]
test_dataset_trofi = TextDataset([example[0] for example in embedded_test_trofi],
                                 [example[1] for example in embedded_test_trofi])
test_dataloader_trofi = DataLoader(dataset=test_dataset_trofi, batch_size=batch_size,
                                   collate_fn=TextDataset.collate_fn)
avg_eval_loss, eval_accuracy, precision, recall, f1, fus_f1 = evaluate(test_dataloader_trofi, rnn_clf,
                                                                       nll_criterion, using_GPU)
print("Test Accuracy {}. Test Precision {}. Test Recall {}. Test F1 {}. Test class-wise F1 {}.".format(
    eval_accuracy, precision, recall, f1, fus_f1))
